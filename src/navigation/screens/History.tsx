...
export function HistoryScreen() {
  ...
  const pts =
    snapshots.filter((s) => s.t >= cutoff).length >= 2
  ...
  const rows = [...snapshots].reverse().map((cur, i, arr) => {
    const prv = arr[i + 1];
    return {
      ...cur,
    ...
    >
      ...
      >
        ...
        <View
          style={[
            styles.card,
            { backgroundColor: theme.cardBg, borderColor: theme.line },
          ]}
        >
          ...
          >
            ...
            {!isPos && !isNeg && (
              <View style={[styles.dash, { backgroundColor: changeColor }]} />
            )}
        ...
        >
          <View style={[styles.tableHeader, { backgroundColor: theme.chipBg }]}>
            <Text style={[styles.colHead, { flex: 1, color: theme.sub }]}>
              DATE
            </Text>
            <Text
              style={[
                styles.colHead,
                { width: 100, textAlign: "right", color: theme.sub },
              ]}
            >
              NET WORTH
            </Text>
            <Text
              style={[
                styles.colHead,
                { width: 80, textAlign: "right", color: theme.sub },
              ]}
            >
              CHANGE
            </Text>
          ...
          {rows.map((r) => {
            ...
            return (
              <View
                key={r.id}
                style={[styles.tableRow, { borderTopColor: theme.line }]}
              >
                <Text style={[styles.rowDate, { flex: 1, color: theme.text }]}>
                  {fmtDateTime(r.t)}
                </Text>
                <Text
                  style={[styles.rowValue, { width: 100, color: theme.text }]}
                >
                  {hideBalance
                    ? "••••"
                    : formatMoney(r.v, baseCurrency, { compact: true })}
                </Text>
                <Text
                  style={[
                    styles.rowChange,
                    { width: 80, color: rowChangeColor },
                  ]}
                >
                  {r.isFirst
                    ? "Start"
                    : (r.change > 0 ? "+" : "") +
                      formatMoney(r.change, baseCurrency, { compact: true })}
                </Text>
              </View>
            );
          })}
